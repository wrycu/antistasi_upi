...
if ((side _unit == buenos) or (side _unit == civilian)) then
	...
	if (side _unit == malos) then
		{
		_otrosMalos = playableUnits select {side _x == malos};
		if (count _otrosMalos > 0) then
			{
			_malo = leader group (_otrosMalos select 0);
			[_malo,[soporteNATO]] remoteExec ["synchronizeObjectsAdd",_malo];
