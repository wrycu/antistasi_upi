...
if (_tipo in SDKSniper) then
	...
	[_unit, "srifle_DMR_06_camo_F", 8, 0] call BIS_fnc_addWeapon;
	...
	if (_tipo in SDKMil) then
		...
		if (_tipo in SDKExp) then
			...
			if (_tipo in SDKEng) then
				...
				{
				if (_tipo in SDKMedic) then {_unit setUnitTrait ["medic",true]};
				};
			};
		};
...
if (player == leader _unit) then
	...
	if (not("ItemRadio" in unlockedItems) and (!hayTFAR)) then
		{
