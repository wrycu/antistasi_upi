...
_tsk1 = ["AtaqueAAF1",_ladosTsk1,[format ["We are attacking %2 from the %1. Intercept them or we may loose a sector",_nombreorig,_nombreDest],format ["%1 Attack",_nombreEny],_mrkDestino],getMarkerPos _mrkDestino,"CREATED",10,true,true,"Attack"] call BIS_fnc_setTask;
...
if (_lado == malos) then
	...
	if ((not(_mrkDestino in puestos)) and (not(_mrkDestino in puertos))) then
		{
		[_mrkdestino,_mrkDestino] spawn artilleria;
...
if (_lado == malos) then
	...
	if  ((({(alive _x) and (!captive _x) and (_x distance _posDestino <= _size)} count _soldados) > 3*({(alive _x) and (!captive _x) and (side _x != _lado) and (side _x != civilian) and (_x distance _posDestino <= _size)} count allUnits)) and (not(_mrkDestino in mrkNATO))) then
		...
		_tsk1 = ["AtaqueAAF1",_ladosTsk1,[format ["We are attacking an %2 from the %1. Intercept them or we may loose a sector",_nombreorig,_nombreDest],format ["%1 Attack",_nombreEny],_mrkDestino],getMarkerPos _mrkDestino,"SUCEEDED",10,true,true,"Attack"] call BIS_fnc_setTask;
		};
	sleep 10;
	if (!(_mrkDestino in mrkNATO)) then
		...
		_tsk1 = ["AtaqueAAF1",_ladosTsk1,[format ["We are attacking an %2 from the %1. Intercept them or we may loose a sector",_nombreorig,_nombreDest],format ["%1 Attack",_nombreEny],_mrkDestino],getMarkerPos _mrkDestino,"FAILED",10,true,true,"Attack"] call BIS_fnc_setTask;
		};
	}
else
	{
	...
	if  ((({(alive _x) and (!captive _x) and (_x distance _posDestino <= _size)} count _soldados) > 3*({(alive _x) and (!captive _x) and (side _x != _lado) and (side _x != civilian) and (_x distance _posDestino <= _size)} count allUnits)) and (not(_mrkDestino in mrkCSAT))) then
		...
		_tsk = ["AtaqueAAF",_ladosTsk,[format ["%2 Is attacking from the %1. Intercept them or we may loose a sector",_nombreorig,_nombreEny],format ["%1 Attack",_nombreEny],_mrkOrigen],getMarkerPos _mrkOrigen,"FAILED",10,true,true,"Defend"] call BIS_fnc_setTask;
		_tsk1 = ["AtaqueAAF1",_ladosTsk1,[format ["We are attacking an %2 from the %1. Intercept them or we may loose a sector",_nombreorig,_nombreDest],format ["%1 Attack",_nombreEny],_mrkDestino],getMarkerPos _mrkDestino,"SUCCEEDED",10,true,true,"Attack"] call BIS_fnc_setTask;
		};
	sleep 10;
...
if ((_lado == malos) or _esSDK) then {[7200] remoteExec ["timingCA",2]} else {[3600] remoteExec ["timingCA",2]};
