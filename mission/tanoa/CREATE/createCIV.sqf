...
for "_i" from 0 to 3 do
	{
	_pos = _posicion getPos [500,(_i*90)];
	if (surfaceIsWater _pos) exitWith
		{
		_esMar = true;
		};
...
if (_esMar) then
	{
	_pos = [_posicion, 50, 500, 10, 2, 0.3, 1] call BIS_Fnc_findSafePos;
	for "_i" from 0 to (round (random 3)) do
		{
		if (spawner getVariable _marcador) then
			{
...
*/
if ((random 100 < ((prestigeNATO) + (prestigeCSAT))) and (spawner getVariable _marcador)) then
	{
	_pos = [];
	while {true} do
		{
