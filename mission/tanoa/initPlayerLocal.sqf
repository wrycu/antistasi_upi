...
if (side player != buenos) exitWith
	{
	if (!_isJIP) then {["noPvP",false,1,false,false] call BIS_fnc_endMission};
	if (not([player] call isMember)) then {["noPvP",false,1,false,false] call BIS_fnc_endMission};
	if (side player == malos) then {player setVariable ["BLUFORSpawn",true,true]};
	"respawn_guerrila" setMarkerAlphaLocal 0;
	if (player == leader group player) then
		{
		player synchronizeObjectsAdd [soporteNATO];
...
caja addEventHandler ["ContainerOpened",
	{
	_fabricas = count (fabricas - mrkNATO);
	hint format ["Arsenal Unlocking Requirements\n\n\nWeapons: %1\nBackpacks: %5\nMagazines: %2\nOptics: %3\nVests: %3\nOther Items: %4",minWeaps,minMags,minOptics,minItems,minPacks];
	}
    ];
waitUntil {scriptdone _introshot};
if (_isJip) then
