...
_chequeo = false;
{_enemigo = _x;
{if (((side _enemigo == muyMalos) or (side _enemigo == malos)) and (_enemigo distance _x < 500) and (not(captive _enemigo))) exitWith {_chequeo = true}} forEach units _grupo;
...
if (count _posicionTel > 0) then
	...
	if (_base in mrkNATO) exitWith {hint "You cannot Fast Travel to an enemy controlled zone"; openMap [false,false]};

	//if (_base in puestosFIA) exitWith {hint "You cannot Fast Travel to roadblocks and watchposts"; openMap [false,false]};

	{
		if (((side _x == muyMalos) or (side _x == malos)) and (_x distance (getMarkerPos _base) < 500) and (not(captive _x))) then {_chequeo = true};
	} forEach allUnits;
