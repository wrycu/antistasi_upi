...
private ["_pos","_timeOut","_posbase","_posdestino","_soldados","_grupos","_vehiculos","_POWS","_tiempofin","_fechafin","_fechafinNum","_veh","_unit","_grupo","_lado"];
...
if ((_destino in aeropuertos) or (_destino in puestos)) then
	...
	if ((garrison getVariable [_destino,0]) > 16) then {_tiposConvoy pushBack "Refuerzos"};
...
for "_i" from 1 to _cuenta do
	...
	if (!_esFIA) then
		...
		if (not(_tipoVehEsc in vehTanks)) then
			...
			_grupoEsc = [_posbase,_lado, _cfg >> _tipogrupo] call BIS_Fnc_spawnGroup;
...
if (_tipoConvoy == "Refuerzos") then
	...
	for "_i" from 1 to 8 do
		...
		_soldado = _grupoEsc createUnit [_tipoSoldado, _posBase, [],0, "NONE"];
		};
...
if (_tipoConvoy == "Municion") then
	...
	if ((_vehObj distance _posdestino < 100) or (dateToNumber date >_fechafinNum)) then
		...
		[position _vehObj,_lado] spawn patrolCA;
...
if (_tipoConvoy == "Money") then
	...
	if (driver _vehObj getVariable ["GREENFORSpawn",false]) then
		...
		if (_vehObj distance _posHQ < 50) then
			...
			[-120*_bonus0] remoteExec ["timingCA",2];
			{if (_x distance _vehObj < 500) then {[10*_bonus,_x] call playerScoreAdd}} forEach (allPlayers - hcArray);
			[5*_bonus,stavros] call playerScoreAdd;
