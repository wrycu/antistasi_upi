...
if (spawner getVariable _marcador) then
	...
	_grupo = [_pos,_lado, _cfg >> _tipoGrupo] call BIS_Fnc_spawnGroup;
	sleep 1;
	...
	_grupo1 = [_pos,_lado, _cfg >> _tipoGrupo] call BIS_Fnc_spawnGroup;
	sleep 1;
	...
	if ((not alive _camion) or ({_x getVariable ["GREENFORSpawn",false]} count crew _camion > 0)) then
		...
		[position _camion,_lado] spawn patrolCA;
