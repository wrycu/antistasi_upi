...
while {true} do
	...
	if (_ciudad in destroyedCities) then
		...
		if (_ciudad in mrkSDK) then
			...
			if (_power) then
				{
				if (_prestigeSDK + _prestigeNATO + 1 <= 100) then {[0,1,_ciudad] spawn citySupportChange};
				}
			else
				{
			...
			if (_power) then
				{
				if (_prestigeNATO + _prestigeSDK + 1 <= 100) then {[1,0,_ciudad] spawn citySupportChange};
				}
			else
				{
	...
	if (_fabrica in mrkSDK) then
		...
		{
		_garrison = garrison getVariable _fabrica;
		if (_garrison > 0) then
			{
			if (_fabrica in mrkNATO) then {_garrison = _garrison - 2} else {_garrison = _garrison - 1};
			if (_garrison < 0) then {_garrison = 0};
			garrison setVariable [_fabrica,_garrison,true];
			};
		};
	} forEach fabricas;
	...
	if (_recurso in mrkSDK) then
		...
		{
		_garrison = garrison getVariable _recurso;
		if (_garrison > 0) then
			{
			if (_recurso in mrkNATO) then {_garrison = _garrison - 2} else {_garrison = _garrison - 1};
			if (_garrison < 0) then {_garrison = 0};
			garrison setVariable [_recurso,_garrison,true];
			};
		};
	} forEach recursos;
	{
	_garrison = garrison getVariable [_x,0];
	if (_garrison > 0) then
		{
		if (_x in mrkNATO) then {_garrison = _garrison - 2} else {_garrison = _garrison - 1};
		if (_garrison < 0) then {_garrison = 0};
		garrison setVariable [_x,_garrison,true];
		};
	} forEach (puestos + aeropuertos - mrkSDK);
	lastIncome = _recAddSDK;
