...
_tipoMuni = if (_lado == malos) then {"12Rnd_230mm_rockets"} else {"32Rnd_155mm_Mo_shells"};
...
if (_posDestino inRangeOfArtillery [[_veh], ((getArtilleryAmmo [_veh]) select 0)]) then
	...
	while {(alive _veh) and ({_x select 0 == _tipoMuni} count magazinesAmmo _veh > 0) and (_mrkDestino in forcedSpawn)} do
		...
		if (count _objetivos > 0) then
			...
			{
			_objetivos = allUnits select {((side _x == _ladoMalos) or (side _x == buenos)) and (_x distance _posDestino <= _size * 2) and (_lado knowsAbout _x >= 1.4)};
			if (count _objetivos > 0) then
				{
				_cuenta = 0;
				{
				if (_x == leader group _x) then
					{
					_cuentaGrupo = {(alive _x) and (!captive _x)} count units group _x;
