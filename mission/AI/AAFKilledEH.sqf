...
_lado = _muerto getVariable "lado";
...
if (!isNil "_marcador") then
	...
	if (_garrison isEqualType 0) then
		{
		garrison setVariable [_marcador,_garrison+1,true];
		};
				...
				if (!isNull _enemy) then
					...
					if ((_x distance _enemy < 50) and (vehicle _x == _x)) then
						...
						if (_x == leader group _x) then
							{
							if (side _enemy == buenos) then
								{
								if (random 1 < 0.1) then
									{
									[position _enemy,side _x] remoteExec ["patrolCA",HCattack];
									};
								};
							};
